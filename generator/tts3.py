...
class TTSPipeline:
    EMAIL = os.getenv("EMAIL")
    PASSWORD = os.getenv("PASSWORD")
    LOG_FILE = os.path.join(os.path.dirname(__file__), "tts3.log")
    ...
    async def run(self, script_lines, out_dir="media/generated/audio"):
        ...
        char_list = [line[0] for line in script_lines]
        tokens = []
        indices = []
        for character in char_list:
            self.log_line(f"STATUS: Converting text to voice for {character}...")

            token, index = get_token_for_character(character, voices)
            tokens.append(token)
            indices.append(index)


        tokens_map = dict(zip(char_list, tokens))
